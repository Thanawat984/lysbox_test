-- 1) Types for finance transactions
DO $$ BEGIN
...
-- Policies (mirror iva_operations logic)
DO $$ BEGIN
  IF NOT EXISTS (
...
-- Optional helper view for chart aggregation
...
-- 3) Share files pivot and extra controls
...
-- Policies for share_files based on share ownership
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    FOR SELECT USING (
      EXISTS (
        SELECT 1 FROM public.shares s WHERE s.id = share_files.share_id AND s.owner_user_id = auth.uid()
      )
    );
  END IF;
  IF NOT EXISTS (
